...
int findInOptab(FILE *fp, char *opcode, char *mnemonic_buf);
int findInSymtab(FILE *fp, char *symbol);
...
int main() {
    ...
    while (strcmp(opcode, "END") != 0) {
        ...
        if (findInOptab(fp3, opcode, mnemonic) == 1) {
            
            loc = findInSymtab(fp4, operand);
            
            if (loc != -1) {
                sprintf(objectcode, "%s%04d", mnemonic, loc);
            } else {
...
int findInOptab(FILE *fp, char *opcode, char *mnemonic_buf) {
    char code[20], mnemonic[25];
    rewind(fp); 
    
    while (fscanf(fp, "%s%s", code, mnemonic) != EOF) {
        if (strcmp(opcode, code) == 0) {
            strcpy(mnemonic_buf, mnemonic); 
            return 1; 
        }
    }
    return 0; 
...
int findInSymtab(FILE *fp, char *symbol) {
    ...
    while (fscanf(fp, "%s%d", sym, &loc) != EOF) {
